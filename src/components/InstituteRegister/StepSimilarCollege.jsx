  ...
  const handleNext = () => {
    ...
    if (notListed) {
      setCollege({
        ...
        onboardingUserId: account?.onboardingUserId || null,
        otpVerified: false,
      });
    ...
    const verificationStatus =
      (selectedCollege.verification_status || "").toLowerCase().trim();

    // ✅ SAVE COLLEGE IN CONTEXT (FOR PREFILL)
    setCollege({
      notListed: false,
    ...
    if (verificationStatus === "verified") {
      onOtpRequired({
        collegeId: selectedCollege.id,
        collegeName: selectedCollege.college_name,
        onboardingUserId: account?.onboardingUserId,
      });
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="max-h-[420px] overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-3 bg-white">
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button
          onClick={handleNext}
          disabled={loading || (!notListed && !selectedCollegeId)}
        >
          Next
        </Button>
      </div>
